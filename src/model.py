...
class UnsupervisedSAGE(object):
    ...
    """
    def __init__(self, placeholders, features, ips_info, degrees, layer_infos, args):
    ...
    def _loss(self):
        # get ips value
        indices = tf.transpose(tf.stack([self.inputs1, self.inputs2]))
        IPS = tf.gather_nd(self.ips_info, indices)
        # reweigh with IPS
        self.loss = loss.xent_loss(self.outputs1, self.outputs2, self.neg_outputs, IPS)
